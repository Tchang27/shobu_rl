...
def elo_from_arena_results(
	...
	This calculation is referenced from Arpad Elo's 1978 publication "The Rating
	of Chessplayers, Past & Present" ISBN 0-668-04721-6. In particular, we follow
	the recommendation of Paragraph 3.33 (a special rating tournament for Rating
	a Group of Unrated Players), using Formula 6 from Section 2.3. D_alpha comes
	from formula 5 on the same page, and D_p is computed inverse to the process
	described in Paragraph 8.94.
	...
	:param elo_mu: The Elo of the "average" player (the mean of the elo
		distribution)
	:param elo_sigma: The standard deviation of the Elo distribution. Arpad says
		to use 200 here.
	"""
	pattern = os.path.join(ingest_dir, "arena_results_*.csv")
	df = None
	for path in glob.glob(pattern):
		df = pd.read_csv(path) if df is None else pd.concat([df, pd.read_csv(path)], ignore_index=True)

	players_list = pd.unique(df[["player_A", "player_B"]].values.ravel())

	player_scores_map = {}
	for player in players_list:
		if player not in player_scores_map:
			player_scores_map[player] = {
				"score": 0,
				"wins": 0,
				"draws": 0,
				"losses": 0,
		...
		# process wins as player A
		games_A = df.loc[df["player_A"] == player]
		player_scores_map[player]["score"] += games_A["A_wins"].sum()
		player_scores_map[player]["score"] += games_A["draws"].sum() * 0.5
		player_scores_map[player]["wins"] += games_A["A_wins"].sum()
		player_scores_map[player]["draws"] += games_A["draws"].sum()
		player_scores_map[player]["losses"] += games_A["B_wins"].sum()
		player_scores_map[player]["tot_games"] += games_A.sum(numeric_only=True).sum()

		games_B = df.loc[df["player_B"] == player]
		player_scores_map[player]["score"] += games_B["B_wins"].sum()
		player_scores_map[player]["score"] += games_B["draws"].sum() * 0.5
		player_scores_map[player]["wins"] += games_B["B_wins"].sum()
		player_scores_map[player]["draws"] += games_B["draws"].sum()
		player_scores_map[player]["losses"] += games_B["A_wins"].sum()
		player_scores_map[player]["tot_games"] += games_B.sum(numeric_only=True).sum()

	M = len(players_list)
	output_stats = []
	for player, scores in player_scores_map.items():
		percentage_score = scores["score"] / scores["tot_games"]
		z_score = NormalDist().inv_cdf(percentage_score)
		D_p = z_score * elo_sigma * math.sqrt(2)
		D_a = D_p * (M-1) / M
		R_p = elo_mu + D_a
		output_stats.append({"player": player, "elo": round(R_p), **scores})

...
from models import Shobu_MCTS_Conv, HISTORY_SIZE
import gc
import psutil
...
class MCTree_Conv:
    def __init__(self, model: Shobu_MCTS_Conv, starting_state: Shobu, device: torch.device):
...
def play_game(model, device, memory: ReplayMemory_MCTS, epoch: int):    
    ...
    while True:
        ...
        mcts = MCTree_Conv(model, board, device)
...
def pickled_play_game(shared_model, buffer, lock, device, seed):
    ...
    np.random.seed(seed)
    local_model = Shobu_MCTS_Conv(device)
...
class Shobu_MCTS_RL:
    ...
    def train(self):
        ...
        # model
        model = Shobu_MCTS_Conv(self.device)
